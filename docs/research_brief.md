...
当前阶段：五天交付版 M0 资料搜集、洞见提炼、核心结论和 helpful methods 候选冻结。本文中的研究假设是资料整理的导航线索，不是已经成立的结论；最终报告必须区分证据支持、初步验证观察和待验证推测。

本项目的主线是调研与经验总结。实验用于验证关键洞见、演示方法和校准边界，不作为项目目标本身。
...
五天交付版额外关注一个工程问题：如何把当前前沿方法转化为可理解、可复用、可验证的洞见、结论和 helpful methods。

## 产出优先级

本阶段产出按以下顺序排序：

1. Insights：具体、可迁移、可验证的洞见，例如“失败样本应被压缩成可编辑证据，而不是只记录分数”。
2. Conclusions：带证据等级、反例和边界的结论，避免把单篇论文或单次样例写成稳定规律。
3. Helpful methods：可直接复用的方法或 playbook，包含适用场景、步骤、指标、风险和误用边界。
4. Anti-patterns：看似合理但容易误导的做法，例如 one-shot rewrite、只看平均分、无界 memory。
5. Validation demos：最小验证或演示，用来校准前四类产出。

上述各类产出的精确定义、区分口径和必填字段以 `docs/insight_field_standard.md` 为准。
...
4. 洞见化：把资料结论转化为 insight cards、核心 conclusions、helpful methods、反模式和风险边界。
5. 初步验证：选择 1-2 个关键洞见或方法做最小验证，避免为了完整性实现过大的 benchmark harness。
6. 最终报告：以 insights、conclusions、helpful methods 为主线，整合前沿状态、行业经验、验证观察和后续路线。
...
- 一份当前前沿状态总结，覆盖代表论文、工具实践、主要争议和可迁移洞见。
- 一份稳定 insight / conclusion / helpful method 清单，覆盖适用场景、步骤、反例、误用风险和验证方式。
- 2-3 个高优先级可复用方法或建议，包含适用场景、指标、成本、风险和回滚策略。
- 1-2 个初步验证或演示设计，包含它要验证的洞见、失败案例和结论限制。
