...
2026-06-12 补充：新增「批次外深读」清单——经典锚点 5 篇、综述 3 篇、对照基线 2 篇不属于任何 batch，批次累计 27 篇 + 批次外 10 篇 = arXiv 渠道深读笔记共 37 篇，全目录见 `docs/literature_map.md`。
...
`Why Prompt Optimization Works...` 和前两批的 TextReg / PrefPO / flawed textual gradients 形成了更完整的警告：
...
| 类别 | 已深读论文 | 当前可用结论 |
| --- | --- | --- |
| critique / textual feedback | ProTeGi, CriSPO, GEPA, Scaling Textual Gradients, Flawed Textual Gradients, VISTA | critique 是候选生成信号，不是真梯度；根因假设空间是关键瓶颈。 |
...
| agent / tool / multi-agent | AutoPDL, JTPRO, MASPO, MAPRO, Temporal/Structural Credit, SPEAR | 优化对象应扩展到 pattern、tool schema、role、round、topology。 |
