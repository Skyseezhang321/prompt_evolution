...
`main` 分支代表项目当前可复现、可审查、可回滚的稳定基线。本轮流程规则变更完成提交并推送后，所有贡献者和 coding agent 在开始一项新任务或一次独立贡献时，先从最新 `main` 创建一条个人或任务分支；这条分支是合入 `main` 的最小单位，可以承载多个 session、多次批量改动和多次提交，全部在自己的分支内修改、验证和提交，不必每次改动都重新从 `main` 建分支。
