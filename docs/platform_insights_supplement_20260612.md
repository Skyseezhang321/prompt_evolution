...
本文把这部分内容按主题集中整理出来,作为主报告的伴读补充。口径如下:
...
| 渠道 | 编号卡片数 | 主线采用 | 仅互证提及 | 未采用 | 本文重点补充的内容 |
|---|---|---|---|---|---|
| 知乎 | 11(ZHI-01~08 + HM-ZH-01~03) | 5 | 5 | 1 | 误读样本库、工具筛选字段、本土工具线索(§4.6、§5、§2.3) |
| Twitter/X | 19(8 insight + 5 conclusion + 6 method) | 13 | 5 | 1 | 研究→工具落地映射、七层失败归因、安全优化门(§3、§4.2、§6) |
| web_search | 14(WPI-01~10 + WHM-01~04) | 7 | 6 | 1* | 八工具五维对标全表、阶段选型依据、字段级操作细节(§2.1、§2.2、§4) |

*web_search 的「未采用」主要不是卡片,而是表格类成果(八工具对标全表)未进报告正文。逐卡明细见 §7 附录;「主线采用 / 仅互证」的判定口径:主线采用 = 在 v4 正文或统一目录作为证据引用;仅互证 = 只出现在跨渠道互证、误区表、工具地图或渠道分述。
...
- **DSPy 误读样本**(ZHI-03,D,目前未被报告采用):中文社区普遍把 DSPy 简化成「自动改提示词工具」,忽略 signature / module / metric 都需手工定义;这是误区 #2 的中文传播实例。
...
1. **tw-insight-07「安全监控 prompt 不能只优化平均分」**(D,未采用):提出 audit budget、monitor failure、safety regression 三类指标,主张「安全边界不应被优化掉」。与 I-10(聚焦 tool schema)关注点错开而非矛盾。值得列为后续专项:主报告的 Safety / Eval / Observability 维度本就弱覆盖(知乎渠道该方向仅约 2 条候选)。
...
| ZHI-03 | DSPy 应解释为 prompt-as-program,不是自动改写器 | D | 未采用 | 中文开发者简化误读样本(见 §5) |
...
| tw-insight-07 | 安全监控 prompt 不能只优化平均分 | D | 未采用 | 安全优化门(见 §6.1) |
