...
- 覆盖边界（2026-06-10 补充）：本判断基于一次受限召回的 discovery（无 token、8 查询）+ core4 审计；`gepa-ai/gepa`、`microsoft/PromptWizard`、`SalesforceAIResearch/promptomatix`、`Eladlev/AutoPrompt` 等正典 optimizer 仓库尚未审计，故”效果证据稀缺”不能排除是覆盖不足所致。详见 [渠道洞见综合 · 渠道覆盖与已知偏差](github_repo_channel_synthesis_20260609.md)。
- 证据路径：
