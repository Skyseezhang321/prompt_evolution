...
状态：候选实验设计，暂不进入完整 benchmark 实现。先完成 M0 资料搜集、前沿状态总结和方案候选，再选择 1-2 个最小实验验证关键判断。

## M0：资料搜集与综述冻结

在实现任何完整 benchmark harness 或自动优化脚本前，先按 [资料搜集计划](source_collection_plan.md) 完成论文、行业经验、工程框架和失败案例收集，并形成可执行方案候选。
...
- 输出 2-3 个可执行方案或建议。
- 输出 1-2 个最小实验候选，每个候选写清目标、假设、输入样本、模型、评估方式、成功标准和主要风险。

未完成 M0 前，不冻结长期任务、数据集或评分器；但可以为报告需要设计一次小规模初步验证。

## 初步验证目标

构建一个小型、可复现的初步验证，观察关键判断是否有支持迹象，同时记录成本、失败模式和过拟合风险。

初步实验不承担“证明某方法长期有效”的任务，只用于支持或修正最终报告中的可执行建议。
...
当前只保留候选，不做长期冻结。M0 完成后，再根据文献和行业经验选择最能验证关键判断的任务，避免过早排除 RAG、agent 或 tool-use 中更有研究价值的最小任务。
