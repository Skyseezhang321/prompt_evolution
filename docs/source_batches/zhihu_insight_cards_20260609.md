...
2026-06-10 修订：(1) `evidence_strength` 由 ZH-L1..L4 收敛到规范 A/B/C/D（见证据边界节），并为每张卡补 `main_sources`；(2) 新增 [ZHI ↔ 全渠道 catalog 交叉对照] 防重复计数；(3) [重点追溯对象] 表接上仓库内已登记 source_id 与已存在的结构化笔记。注：保留候选登记进 `source_inventory.md` 与 `CHANGELOG.md` 条目因该文件正被其它渠道并发编辑，本次未一并改动，留待其干净后补。
...
**evidence_strength 统一用规范的 A/B/C/D**（定义见 [insight_field_standard.md](../insight_field_standard.md) 第四节）。知乎渠道此前使用的 ZH-L1..L4 只表示”溯源进度”，不是独立的证据强度标度，按下表映射到规范等级；各卡正文的 `evidence_strength` 字段以映射后的 A/B/C/D 为准，不再使用 ZH-Lx 取值。

| 溯源进度（ZH-Lx） | 含义 | 映射到 evidence_strength | 当前用途 |
| --- | --- | --- | --- |
| ZH-L1 | 仅基于搜索标题、摘要、URL 和主题聚类 | D | 发现中文社区问题意识、传播口径和候选追溯对象 |
| ZH-L2 | 已读全文，且本地记录快照路径、SHA256 和结构化摘要 | D（单一二手来源，仍不证明效果） | 可写行业经验笔记 |
| ZH-L3 | 已追溯到原论文、官方文档、代码或可运行示例 | B（一手来源已做结构化笔记时可达 A） | 可支持方法背景和证据链 |
| ZH-L4 | 与其它渠道或本项目实验互相印证 | A | 可进入最终报告的核心 conclusion |

批次说明：本批多数卡片溯源进度仍为 ZH-L1（即 evidence_strength D）；少数已能指向仓库内已提交的结构化笔记（如 GEPA、MemAPO/SePO）记为 B。各卡 `main_sources` 给出当前可追溯到的仓库内文件或 source_id；溯源补齐前，凡涉及”超过 RL””大幅提升””自动进化””生产可用”等主张都不得写入最终报告核心结论。
...
| id | 候选 insight | 主要来源簇 | evidence_strength（溯源进度 / 补证路径） | 最小验证优先级 |
| --- | --- | --- | --- | --- |
| ZHI-01 | prompt optimization 已从技巧问题转向 eval-driven iteration。 | APO/APE/OPRO/EvoPrompt/PRewrite 中文解读 | B（已有 coin-flip / ProTeGi 笔记；追溯 APO 谱系可升 A） | high |
| ZHI-02 | GEPA 的可迁移洞见是 trace-aware reflection，而不是”全面超过 RL”。 | GEPA / reflective prompt evolution 文章 | B（GEPA 笔记已支撑） | high |
| ZHI-03 | DSPy/MIPRO 应被写成 prompt-as-program，而不是自动提示词生成器。 | DSPy / MIPRO / Prompt 编译文章 | D（DSPy/MIPRO paper note 待补，inventory 已登记） | high |
| ZHI-04 | context engineering 把优化对象从单条 prompt 扩展到模型可见上下文。 | Context Engineering / agent context / RAG context | D（context engineering 工程资料待笔记） | high |
| ZHI-05 | 工具类文章的筛选标准是是否支持可复盘评估闭环。 | OPIK / Prompt Optimizer / PromptPilot / Coze | D（工具官方文档待笔记） | medium |
| ZHI-06 | agent self-evolution 必须先定义可变对象和冻结对象。 | Hermes / Manus / Skill / Agent 自进化 | B（MemAPO / SePO 笔记已支撑） | high |
| ZHI-07 | 中文来源适合提炼用户语言和误区，不适合直接承载性能结论。 | 全批次综合 | D（渠道级方法论判断） | medium |
| ZHI-08 | 泛 prompt 技巧是反模式线索，除非它能接入 eval 和版本管理。 | 泛 prompt engineering 文章 | D（接入 catalog I-07/I-12 后判定） | medium |

## ZHI ↔ 全渠道 catalog 交叉对照（防重复计数）

知乎 ZHI 卡片与研究者向 [insight_method_catalog](../insight_method_catalog_20260609.md) 的 I- / HM- 编号是两套命名空间。融入最终报告的全渠道 insight pool 时，必须按下表去重，避免把同一洞见的中文传播版本当成独立洞见重复计数。`关系` 取值：`duplicate`（同一洞见）、`extension`（补充已有洞见的边界/语言/案例）、`new`（catalog 暂无）。

| 知乎条目 | 对应 catalog | 关系 | 知乎渠道的净增量 |
| --- | --- | --- | --- |
| ZHI-01 | I-01、I-02、I-11 | duplicate | 中文社区"找万能模板"痛点的用户语言 |
| ZHI-02 | I-02 | extension | "超越 RL"标题被误读的传播口径 |
| ZHI-03 | I-06（含 I-05） | duplicate | "自动写提示词"简化误读 |
| ZHI-04 | I-06 | duplicate | context engineering 在中文圈的讨论密度 |
| ZHI-05 | I-12、I-04、HM-04 | duplicate | 工具体验文的营销话术样本 |
| ZHI-06 | I-08、I-06 | extension | agent 自进化"可变对象未拆分"的中文叙述 |
| ZHI-07 | I-12 | duplicate | 渠道级印证（中文二手=线索，不承载性能结论） |
| ZHI-08 | I-07、I-12 | extension | 泛技巧降权的用户痛点表达 |
| HM-ZH-01 | HM-01 | duplicate | — |
| HM-ZH-02 | HM-02 | duplicate | — |
| HM-ZH-03 | （无对应） | new | 二手 post 的 primary-source promotion gate，catalog 暂无 |

结论：知乎渠道与 catalog 高度重合，绝大多数是 `duplicate`/`extension`；唯一净新增的可复用方法是 **HM-ZH-03（primary-source promotion gate）**。最终报告中 ZHI-01~08 应作为对应 I- 洞见的"中文传播 / 误区 / 用户语言"补充材料引用，不单独成条计数。
...
- evidence_strength：B（溯源进度 ZH-L1，但机制已有结构化笔记支撑）。与更多 arXiv/GitHub/其它平台资料一致时可升 A。
...
- evidence_strength：B（溯源进度 ZH-L1，GEPA 结构化笔记已支撑该机制）。本项目复现后可升 A。
...
- evidence_strength：D（溯源进度 ZH-L1，DSPy/MIPRO 仅 inventory 登记，结构化 paper note 待补）。
...
- evidence_strength：D（溯源进度 ZH-L1，需 context engineering 工程资料结构化后补证）。
...
- evidence_strength：D（溯源进度 ZH-L1，需工具官方文档或本地运行记录结构化后补证）。
...
- evidence_strength：B（溯源进度 ZH-L1，MemAPO/SePO 结构化笔记已支撑"可变对象拆分"机制）。Hermes 项目源码、issues、运行 trace 核验后可升 A。
...
- evidence_strength：D（溯源进度 ZH-L1；这是渠道级方法论判断，本身即对应 catalog I-12）。
...
- evidence_strength：D（溯源进度 ZH-L1；接入 catalog I-07/I-12 后判定）。
...
下表把每个追溯对象接到 `docs/source_inventory.md` 已登记的 source_id 和（若已存在）`docs/paper_notes/` 结构化笔记，使 ZHI 卡片从"应追溯"变为"已指向具体仓库内一手来源"。

| 对象 | 从知乎得到的线索 | 仓库内一手来源（source_id / 笔记路径） | 当前溯源状态 | 进入最终报告的角色 |
| --- | --- | --- | --- | --- |
| GEPA | 反思式 prompt evolution、trace feedback、与 RL 对比 | `paper-gepa-2025`；笔记 [paper-gepa-2026.md](../paper_notes/paper-gepa-2026.md) | 已有结构化笔记（B） | 核心 insight 和验证候选 |
| DSPy / MIPRO | prompt-as-program、signature、optimizer、metric | `paper-dspy-2023`、`paper-miprov2-2024`（inventory 已登记，paper note 待补） | 仅 inventory 登记（D） | helpful method 背景 |
| APO / ProTeGi / OPRO / EvoPrompt | 自动提示工程方法索引 | `paper-protegi-2023`（笔记 [paper-protegi-2023.md](../paper_notes/paper-protegi-2023.md)）、`paper-evoprompt-2023`（笔记 [paper-evoprompt-2024.md](../paper_notes/paper-evoprompt-2024.md)）、`paper-ape-2022`、`paper-opro-2023`、`paper-prewrite-2024` | ProTeGi/EvoPrompt 已有笔记（B），其余仅登记（D） | 方法 taxonomy |
| Context engineering | prompt 到 context/workflow 的边界扩展 | `paper-context-engineering-2025`、`practice-anthropic-context-engineering`、`practice-langchain-agent-context-engineering` | 仅 inventory 登记（D） | 工程结论和反模式 |
| Hermes / Manus / Skill 自进化 | agent 经验沉淀、skill、memory 和自进化 | `practice-zhihu-hermes-agent-2026`（笔记 [practice-zhihu-hermes-agent-2026.md](../industry_notes/practice-zhihu-hermes-agent-2026.md)）、`paper-memapo-2026`、`paper-sepo-2026` | Hermes 笔记 weak + MemAPO/SePO 笔记（B） | 自进化变量拆分线索 |
| OPIK / Prompt Optimizer / PromptPilot / Coze | 工具化需求、本土生态、产品工作流 | `practice-opik-optimizer-overview`、`practice-promptfoo-optimization`、`repo-linshenkx-prompt-optimizer` | 官方 docs / 源码已登记，待笔记（D） | 行业工具地图 |
