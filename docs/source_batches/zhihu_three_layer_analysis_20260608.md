...
2026-06-10 补充两点：(1) 本文件各主题表中的"证据等级"（weak/medium/unknown）是快筛期粗判，仅供排序；权威 evidence_strength 以 [洞见与方法卡片](zhihu_insight_cards_20260609.md) 收敛后的规范 A/B/C/D 为准。(2) 渠道级覆盖缺口：本批 Safety / Eval / Observability 仅约 2 条候选，明显低于覆盖矩阵对 eval/治理 的门槛（8 条）；知乎渠道在该方向应判定为弱覆盖，相关证据须从官方工具文档与其它渠道补齐，不在本渠道强行凑数。
