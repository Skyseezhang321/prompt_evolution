...
function generate(){
  ...
  if (LLM.available){
    ...
    botSay('上面是确定性命中的完整卡片。下面我再<b>结合你的场景做个总结</b>，把适用洞见串成一份行动计划（LLM 流式生成、扎根知识库）👇', false);
    llmAsk('请结合我的场景，把适用的洞见总结成一份按优先级排序的行动计划（3–5 步）：每一步给出贴合我场景的具体例子或方向，并标注洞见引用与证据等级。');
  } else {
    botSay('建议看完了。你可以继续追问任意话题，我会从知识库里捞相关洞见（当前为关键词匹配，<b>非 LLM</b>；要用 LLM 总结与自由问答，请按 advisor/README.md 起后端并访问 http://localhost:8000/）。', false);
  }
  freeChips();
}
function freeChips(){
...
const RESOURCES = [
  ...
  { t: '分享 Deck（1 小时演示稿 · 20 页）', p: 'docs/share_deck_20260612.html' },
  { t: '项目一页纸总览（可打印）', p: 'docs/project_onepager_20260612.html' },
];
...
const CHANNEL_REPORTS = [
  ...
  { t: '知乎（D）· 洞见卡', p: 'docs/source_batches/zhihu_insight_cards_20260609.html' },
];
