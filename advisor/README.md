...
页面是一个三栏布局，一个入口看全部内容：**左**＝知识库导览（文档/报告链接 + 12 洞见分组 + 反模式 + 首批实验，点条目→右侧看详情）；**中**＝对话；**右**＝详情面板（默认证据等级图例 + 诚实声明；点左侧条目显示其全文卡；每次回答后显示「本轮回答引用的来源」，可点洞见再展开）。窄屏（<1180/<860）左右面板收为抽屉，由顶栏「📚 知识库 / 📄 详情」按钮唤出。
